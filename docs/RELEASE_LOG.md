## 2026-09-24 · qidian-next 1.2.8-beta1 — 书旗严格书籍/章节匹配
...
## 2026-09-14 · qidian-next 1.2.7-beta1
...
## 2026-09-14 · qidian-next 1.2.5-beta2 — 版权信息阅读原生单页版
...
## 2026-09-14 · qidian-next 1.2.5-beta1 — 版权信息页视觉优化
...
## 2026-09-12 · qidian-next 1.2.1-beta3
- 情无/小雨用户系统按当前附件重接：正文自动游客 Token + 手工游客授权 + 网页注册/登录。
- 正文认证统一为 `X-Sec-Token` / `X-Android-Id`；移除旧邮箱密码直登假设。
- Stable 1.2.0 与其它功能域不变。
...
## 2026-09-11 · 起点增强 1.1.0-beta40 — 情无章节解码修复 / 账号入口移除
...
## 2026-09-11 · 起点增强 1.1.0-beta39 — 新情无章节匹配 / 免登录
...
## 2026-09-11 · 起点增强 1.1.0-beta38 — 情无来源替换 / 尾注净化
...
## 2026-09-10 · 起点增强 1.1.0-beta37 — 评论点赞定位修复
...
## 2026-09-10 · 起点增强 1.1.0-beta36 — 评论页密度第二轮
...
## 2026-09-10 · 起点增强 1.1.0-beta35 — 点赞归位到元信息行
...
## 2026-09-10 · 起点增强 1.1.0-beta34 — 评论页信息密度优化

- Stable 保持 1.1.0 不变。
- 根评论点赞区域收窄并贴近右侧，减少 `mainRow` 的右侧预留，让长评论获得更多横向显示空间。
- 楼中楼 `rLike` 同步缩窄，回复卡片右侧 padding 减少；点赞图标和数字仍完整保留。
- 仅修改 lazy `review_local_ui` 的 CSS；评论数据、接口、排序、身份标签、楼中楼分页及 X-QD Provider 均冻结。
- 状态：Beta，等待真机确认。
...
- 通道：RSS UI Beta；当前订阅 identity 不变。
- 新增 `📚 小说` 类型分类，位置固定在首页与漫画之间。
- 新建 `subscription/novel.json`，当前收录起点官方生态与起点增强当前活动版本。
- 原 `📰 仓库订阅` 改名为 `📰 订阅`，并移动到 `📦 批量` 前。
- 后续发布规则：书源除 Stable/Beta 状态目录外，还必须同步对应类型目录；同一逻辑书源在类型目录中只保留当前活动版本。
- 当前分类顺序：首页 → 小说 → 漫画 → 正式版 → 测试版 → 订阅 → 批量 → 帮助。

## 2026-08-29 · 🌈 起点增强 1.1.0-beta31
