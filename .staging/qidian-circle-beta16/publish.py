...
# Add in-memory preview map before feedCard. The detail button itself stays compact/legacy-compatible.
marker='    h.push("function feedCard(o,label){'
assert code.count(marker)==1
code=code.replace(marker,'    h.push("var qfPostPreviewImgs={};");\n'+marker,1)
# Capture full image list after direct list images are merged.
...
assert 'qfPostPreviewImgs' in code and 'state.previewImgs=pic' in code
...
src['bookSourceComment']='v1.1.0-beta16：书友圈详情点击热修。撤销 Beta15 将完整图片数组塞入 data-imgs DOM 属性的做法，恢复已验证的紧凑详情按钮；多图改为按 postId 保存在当前 WebView 内存映射，点击后再合并详情接口图片。Beta15 投票元数据精确清理继续保留，其它域冻结。'
...
assert 'qfPostPreviewImgs' in cc and 'data-imgs=' not in cc and 'qfCleanPollText' in cc and 'mergedImgs' in cc
...
changes=['恢复 Beta14 已验证的紧凑“查看详情”按钮 DOM，不再把完整图片数组编码进 data-imgs 属性','完整列表配图改为按 postId 保存在当前书友圈 WebView 内存映射，点击详情时读取并与 getpostdetail 图片去重合并','保留 Beta15 的 Options/VoteId/VoteType 投票尾元数据精确清理','评论/楼中楼、分类、视频、详情、正文 Provider、角色卡、账号链冻结']
