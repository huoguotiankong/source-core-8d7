...
# so label/radio and normal click synthesis could never complete.  Tab touches must bypass
# the custom scroll/swipe recognizer entirely.
pat=re.compile(r'function bindInfo\(\)\{var ins=document\.querySelectorAll\("\\\.info"\);.*?\}\}bindInfo\(\);',re.S)
mm=pat.search(role)
assert mm,'bindInfo block not found'
...
role=role[:mm.start()]+fixed+role[mm.end():]

# Beta22's JS tab binder is obsolete and conflicts with the radio/label implementation.
role=re.sub(r'function bindTabs\(s\)\{.*?\}\}function paint\(\)', 'function bindTabs(s){return}function paint()', role, count=1, flags=re.S)
role=role.replace('ensureImg(i);bindTabs(ss[i]);','ensureImg(i);',1)

# Explicitly mark the tab strip as a native tap zone.
