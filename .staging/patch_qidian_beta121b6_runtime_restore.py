...
def main():
    ...
    # Rebuild from the verified Beta4 source state. Beta5 jsLib is not used as a code baseline.
    ...
    if "GZIPInputStream" not in old_loader:
        raise SystemExit("unexpected Beta4 unpack function")
    ...
    # Real newlines, not the literal backslash+n sequence that Beta5 accidentally inserted.
    loader_lines = [
    ...
    src["bookSourceComment"] = (
        "v1.2.1-beta6：紧急修复 Beta5 回归。恢复已验证 Beta4 完整运行时，仅在 qfModuleUnpackV41 内保留 Base64URL 解码兼容；"
        "补回 Beta5 误删的 alpha84 并发运行时，修复 qfModuleThreadIdV84 未定义；移除 Beta5 错误写入的字面量反斜杠+n，修复正文 Rhino 非法字符。"
        "情无/小雨评论、正文链、账号系统和其它 Provider 均保持 Beta4 逻辑不变。Stable 1.2.0 不变。"
    )
    ...
    # Hard isolation gate: jsLib outside the unpack function must equal Beta4 byte-for-byte.
    new_start = js.find(fn)
    new_end = js.find(marker, new_start)
    if base_js[:start] != js[:new_start]:
        raise SystemExit("unexpected jsLib change before loader")
    if base_js[end:] != js[new_end:]:
        raise SystemExit("unexpected jsLib change after loader / alpha84 runtime")
    ...
    if js.count("function qfModuleThreadIdV84(){") != 1:
        raise SystemExit("qfModuleThreadIdV84 definition count invalid")
    if js.count("function qfModuleWaitV84(name,owner){") != 1:
        raise SystemExit("qfModuleWaitV84 definition count invalid")
    if js.count(fn) != 1:
        raise SystemExit("qfModuleUnpackV41 definition count invalid")
    if "function qfModuleUnpackV41(v){\\n" in js:
        raise SystemExit("literal backslash+n still present at loader start")
    if fn + "\n" not in js:
        raise SystemExit("loader does not contain a real newline")

    SRC.write_text(json.dumps(arr, ensure_ascii=False, indent=2), encoding="utf-8")

    # Validate all 31 lazy modules using the same Base64URL normalization.
    m = re.search(r"var\s+QF_MOD38_PACK\s*=\s*(\{.*?\});\s*var\s+QF_MOD38_EXPORTS", js, re.S)
    if not m:
        raise SystemExit("module pack not found")
    ...
    for name, val in pack.items():
        ...
        if text.startswith("\\n") or text.startswith("\\r"):
            raise SystemExit(f"module {name} starts with a literal backslash escape")
        checked += 1
    if checked != 31:
        raise SystemExit(f"expected 31 compressed modules, got {checked}")

    # Parser smoke test catches a stray backslash before publishing.
    node_script = (
        'const fs=require("fs");'
        'const a=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));'
        'const s=Array.isArray(a)?a[0]:a;'
        'new Function(s.jsLib);'
        'console.log("jsLib syntax OK");'
    )
    with tempfile.NamedTemporaryFile("w", suffix=".js", delete=False, encoding="utf-8") as tf:
        tf.write(node_script)
        npath = tf.name
    try:
        cp = subprocess.run(["node", npath, str(SRC)], capture_output=True, text=True)
        if cp.returncode != 0:
            raise SystemExit("jsLib syntax check failed: " + cp.stderr[:2000])
    ...
    summary = "Beta5 紧急回归修复：恢复 Beta4 完整运行时，仅保留 Base64URL 解码兼容；修复正文非法反斜杠与评论运行时函数丢失。"
    changes = [
        "修复正文偶发 EvaluatorException：不允许的字符（反斜杠）",
        "恢复 Beta5 误删的 alpha84 并发加载运行时（qfModuleThreadIdV84 / qfModuleWaitV84）",
        "保留 Base64URL -/_ 与缺失 = 补位兼容，31 个压缩模块全部解压校验",
        "除 qfModuleUnpackV41 外 jsLib 与已验证 Beta4 基线逐字节一致",
        "情无/小雨评论、正文链、账号系统及其它 Provider 保持 Beta4；Stable 1.2.0 不变",
    ]
    ...
    def update_item(v):
        ...
        for tag in ["起点", "测试版", "限免源", "情无", "小雨用户系统", "游客Token", "网页登录", "免登录回退", "X", "评论页", "Base64URL", "运行时回归修复"]:
            if tag not in tags:
                tags.append(tag)
        v["tags"] = tags
    ...
    detail = {
        "kind": "source",
        "title": "🌈 起点增强 · Beta",
        "summary": "Beta 1.2.1-beta6：紧急回归修复。恢复 Beta4 完整运行时，仅保留隔离的 Base64URL 解码兼容。",
        "badges": ["Beta", VERSION, "运行时回归修复"],
        "sections": [
            {"title": "正文修复", "text": "移除 Beta5 误写进 jsLib 的字面量反斜杠+n，修复 Rhino 不允许的字符错误；正文 Provider 逻辑回到 Beta4 基线。"},
            {"title": "评论修复", "text": "恢复 Beta5 误删的 alpha84 并发加载函数 qfModuleThreadIdV84 / qfModuleWaitV84，解决点击段评/本章说 ReferenceError。"},
            {"title": "Base64URL", "text": "仅在 qfModuleUnpackV41 内兼容 -/_ 和缺失 =；31 个压缩懒模块全部完成 Base64 解码与 gzip 解压校验。"},
            {"title": "冻结范围", "text": "情无/小雨评论业务、账号系统、正文链及其它 Provider 不改；Stable 1.2.0 不变。"},
        ],
        "sourceUrl": raw_url,
        "backupUrl": cdn_url,
...
"""
    ...
    print("Beta6 ready", "modules=", checked, "sha256=", sha, "time=", now)
