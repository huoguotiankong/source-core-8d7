...
function qfReviewUiIdentity(item,raw){
    ...
    return {
        ...
        avatar:qfNormAssetUrlV1272(String(
...
function qfEmojiOfficialPageV1275(){
    ...
    var map={};
    try{
        var req=qfReaderSignedRequestV3245('v2/chapterreview/getparagraphscomments',{
...
function qfNormAssetUrlV1272(u){
...
function qfAudioCommentHtml(c){
    ...
    var h='<div class="comment audioComment '+(currentTab==='audio'?'audioTabCard':'allAudioCard')+'" data-id="'+esc(c.id)+'">'+
        ...
        '<div class="content qfFoldable qfFolded">'+fmt(c.content,c.emojiHints)+'</div><button class="foldBtn hidden" type="button">展开全文</button>';
...
function commentHtml(c){
    ...
    var h='<div class="comment" data-id="'+esc(c.id)+'">'+
        '<div class="mainRow"><div class="avatarWrap">'+
        avHtml(c,false)+
        (c.frame?'<div class="frame"><img src="'+esc(qfNormAssetUrlV1272(c.frame))+'" referrerpolicy="no-referrer" onerror="this.style.display=\'none\'"></div>':'')+
        '</div><div class="body"><div class="nameRow">'+
        '<span class="name">'+esc(c.name)+'</span>'+
        '<span class="badges">'+b+'</span></div>'+
        '<div class="content">'+fmt(c.content,c.emojiHints)+'</div>';
...
function replyHtml(c){var b='';for(var i=0;i<c.titles.length&&i<2;i++)b+=badgeHtml(c.titles[i]);var h='<div class="reply" data-rid="'+esc(c.id||'')+'"><div class="replyHead">'+avHtml(c,true)+'<span class="rName">'+esc(c.name)+'</span><span class="rBadges">'+b+'</span></div><div class="rContent qfFoldable qfFolded">'+(c.replyTo?'<span class="replyTo">回复 '+esc(c.replyTo)+'：</span>':'')+fmt(c.content,c.emojiHints)+'</div><button class="foldBtn hidden" type="button">展开全文</button>';if(c.image)h+='<img class="mediaImg zoom" style="margin-left:36px;max-width:55vw" src="'+esc(c.image)+'" referrerpolicy="no-referrer" loading="lazy" decoding="async">';if(c.audioPage){
...
function qfReviewCopyTitleV502(dst,src){
    ...
    if(su&&du){var ks=['UserName','userName','NickName','nickName','UserHeadIcon','userHeadIcon','UserAvatar','userAvatar','AvatarUrl','avatarUrl','HeadIcon','headIcon','HeadUrl','headUrl','FaceUrl','faceUrl','Avatar','avatar','UserId','userId','UserID','userID'];for(var i=0;i<ks.length;i++){var k=ks[i];if((du[k]===undefined||du[k]===null||du[k]==='')&&su[k]!==undefined&&su[k]!==null)du[k]=su[k];}}
    return dst;
}
...
function qfReviewFusePackV502(structPack,richPack){
    ...
    for(var j=0;j<structPack.list.length;j++){
        ...
        if(m){
            ...
            qfReviewCopyReaderContentV1278(st,m);
...
function qfAppV2ParagraphPack2971(p,ps,extra){
    ...
    function pullReader(profile,pidValue){
        try{
            var params={bookId:String(bid),chapterId:String(cid),paragraphId:String(pidValue),pg:String(Math.max(1,Number(p)||1)),pz:String(Math.max(1,Number(ps)||10)),type:'0',anchorId:'0',from:'0'};
    ...
    function preferred(){return 'reader_ext'}
...
/* 1.2.4-beta2：Reader v2 首屏快速结构化。仅当官方返回明确 ReviewType/RefferCommentId 结构时启用；否则无条件回退原稳定链。 */
...
var QF_READER_DONOR_DEVICES_V1277=[
...
function qfReplyV2Page2971(rootId,pg,pz){
    ...
    function prefProfile(){return "reader_ext";}
...
function loadMoreReplies(rootId,host,btn,total){
    ...
    function renderRows(out,pk,pageNo){
        ...
        if(remain<=0){btn.outerHTML='<div class="replyDone">已加载全部回复</div>';}
...
while(z&&z!==listEl&&!(z.classList&&z.classList.contains('replyToggle')))z=z.parentNode;if(z&&z.classList&&z.classList.contains('replyToggle')){var id=z.getAttribute('data-reply'),rootId=z.getAttribute('data-root')||id,host=document.getElementById('rp_'+id),card=z.parentNode,c=card?card.querySelector('.comment'):null;var total=Number(z.getAttribute('data-total')||0);if(!total){var m=z.textContent.match(/(\d+)/);if(m)total=Number(m[1])}loadReplies(rootId,host,z,total)}};document.addEventListener('error',function(ev){try{var el=ev&&ev.target;if(el&&el.tagName==='IMG'){if(el.classList&&el.classList.contains('zoom'))return;el.style.display='none';}}catch(_e){}},true);
document.getElementById('lightbox').onclick=function(){this.classList.add('hidden')};
