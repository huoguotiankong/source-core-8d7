...
def splice_between(text, start_marker, end_marker, replacement):
...
def replace_once(text, old, new, label):
    n = text.count(old)
    assert n == 1, f'{label}: expected 1 hit, got {n}'
...
# 1) 最近更新时间可信链：只过滤空值和明显未来值，不再用 60 天阈值误杀旧书，
#    也不再拿首发/上架时间反向判定更新时间。
new_guard = r"""function qfTimeSuspiciousV1111(){
...
s = replace_once(
    ...
    'initial update-time clearing',
)

s = replace_once(
    s,
    "var needIntro=blank(info.intro),needRead=blank(info.readingCount),needRec=blank(info.recommendCount),needUpdate=qfTimeSuspiciousV1111();if(!needIntro&&!needRead&&!needRec&&!needUpdate)return;",
    "var needIntro=blank(info.intro),needRead=blank(info.readingCount),needRec=blank(info.recommendCount),needUpdate=(!qfTimesV1111.updateTime)||qfTimeSuspiciousV1111();if(!needIntro&&!needRead&&!needRec&&!needUpdate)return;",
    'official-search update need',
)
...
# 2) 作品数据：每两个指标明确一行，用真正的 <br> 换行，不再依赖 pre / 等宽字体 / CSS 列宽。
...
assert '<pre style=' not in s, 'pre layout remains after patch'

anchor = s.index('var sc=score(x.ratingScore)')
data_start = s.index('var data=[', anchor)
data_end = s.index('var dr=dataRows(data);', data_start)
new_data = """var data=[
...
# 3) 详情作品资料只显示“最近更新”，彻底去掉“首发”展示。
s = replace_once(
    s,
    "if(clean(x.updateTime))body+=row('🕒','更新',date(x.updateTime));",
    "if(clean(x.updateTime))body+=row('🕒','最近更新',date(x.updateTime));",
    'visible update row',
)
s = replace_once(
    s,
    ...
    '',
    'visible first-publish row',
)

# 4) Beta12 的 qfMultiContentV423 只存在于 loginUrl，详情页作用域不可见。
#    回退为已在 jsLib 验证的全局“正文源状态”入口，先消除真机 ReferenceError。
s = replace_once(
    s,
    '<button>⚡ 正文设置@onclick:qfMultiContentV423.call(this)</button>',
    '<button>⚡ 正文源状态@onclick:qfBookInfoOpenSmartSourceV330.call(this)</button>',
    'broken content settings shortcut',
)

rule['intro'] = s
src['ruleBookInfo'] = rule
src['bookSourceComment'] = (
    'v1.1.0-beta13：详情页时间与双列回归修复。作品资料只显示“最近更新”，移除“首发”展示；'
    '更新时间不再因“连载超过60天”被清空，仅保留空值/未来时间保护。作品数据按两项一行真实换行，'
    '主指标优先固定为总推荐/月票、收藏/粉丝、盟主/首订，其余指标继续两项一行补位。'
    'Beta12 跨作用域“正文设置”按钮回退为已验证的“正文源状态”入口，避免 qfMultiContentV423 未定义。'
    '搜索、目录、正文 Provider、评论、角色卡、书友圈、账号链冻结。'
)
...
# 5) Beta Bundle：只替换当前起点增强 Beta 对象，不碰其它书源。
...
changes = [
    ...
    '作品数据放弃 pre/monospace 与 CSS 列宽，改成每两个指标用真实 HTML 换行；主指标优先为总推荐/月票、收藏/粉丝、盟主/首订',
...
detail = {
    ...
    'sections': [
        {
            'title': '最近更新时间',
            'text': '详情页只保留“最近更新”。取消 60 天陈旧阈值，仅过滤空值和明显未来时间；刷新 v1113 时间缓存，避免旧错误继续命中。',
        },
        {
            'title': '作品数据',
            'text': '不再使用 pre、等宽字体或 CSS 固定列宽。主指标按“总推荐 / 月票、收藏 / 粉丝、盟主 / 首订”成对显示，其余可用指标继续两项一行补位。',
        },
        {
            'title': '正文源入口',
            'text': 'Beta12 的正文设置按钮跨作用域调用 qfMultiContentV423 会报未定义；本版回退到已验证的“正文源状态”全局入口。',
        },
        {
            'title': '冻结范围',
            'text': '搜索、目录、正文 Provider 实际解析、评论、角色卡、书友圈、账号和情无 VIP 链不改。',
        },
    ],
    'sourceUrl': raw,
    'backupUrl': cdn,
